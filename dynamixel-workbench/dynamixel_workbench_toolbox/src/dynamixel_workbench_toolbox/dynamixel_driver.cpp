...
bool DynamixelDriver::clearMultiTurn(uint8_t id, const char **log)
{
  ...
  {
    if (log != NULL)
      *log = "[DynamixelDriver] Succeeded to clear!";
    return true;
  }
...
{
  uint16_t num = getModelNumber(id, log);
  setPacketHandler(1.0f, log);
...
bool DynamixelDriver::getSyncReadData(uint8_t index, int32_t *data, const char **log)
{
  ...
  if (log != NULL)
    *log = "[DynamixelDriver] Succeeded to get sync read data!";
  return true;
}
...
bool DynamixelDriver::addBulkReadParam(uint8_t id, uint16_t address, uint16_t length, const char **log)
{
  ...
  {
    if (log != NULL)
      *log = "[DynamixelDriver] Too many bulk parameter are added (default buffer size is 10)";
    return false;
  }
...
bool DynamixelDriver::getBulkReadData(int32_t *data, const char **log)
{
  ...
  if (log != NULL)
    *log = "[DynamixelDriver] Succeeded to get bulk read data!";
  return true;
}
