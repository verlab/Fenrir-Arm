...
{
  const char *log;
  dynamixels_states_.dynamixel_state.resize(254);
...
bool DynamixelController::getPresentPosition(std::vector<std::string> dxl_name)
{
  ...
  if (dxl_wb_->getProtocolVersion() == 2.0f && false)
  {
  ...
  else if (dxl_wb_->getProtocolVersion() == 1.0f && true)
  {
    WayPoint wp;
    uint32_t read_position;
    for (auto const &dxl : dynamixel_)
    {
      const ControlItem *pos_control;
      const ControlItem *pos_velocity;
      const ControlItem *pos_current;
      uint16_t num = dxl_wb_->getModelNumber((uint8_t)dxl.second, &log);
      if (num == MX_28_2 || num == MX_64_2 || num == MX_106_2 ||
          num == XL430_W250 || num == XL430_W250_2 || num == XC430_W150 ||
          num == XC430_W240 || num == XM430_W210 || num == XM430_W350 ||
          num == XM540_W150 || num == XM540_W270 || num == XH430_V210 ||
          num == XH430_V350 || num == XH430_W210 || num == XH430_W350 ||
          num == XH540_W150 || num == XH540_W270 || num == XH540_V150 ||
          num == XH540_W270 || num == XW540_T260 || num == XW540_T140)
      {
        pos_control = dxl_wb_->getItemInfo((uint8_t)dxl.second, "Present_Position");
        pos_velocity = dxl_wb_->getItemInfo((uint8_t)dxl.second, "Present_Velocity");
        pos_current = dxl_wb_->getItemInfo((uint8_t)dxl.second, "Present_Current");
      }
      else
      {
        pos_control = dxl_wb_->getItemInfo((uint8_t)dxl.second, "Present_Position");
        pos_velocity = dxl_wb_->getItemInfo((uint8_t)dxl.second, "Present_Speed");
        pos_current = dxl_wb_->getItemInfo((uint8_t)dxl.second, "Present_Load");
      }
      result = dxl_wb_->readRegister((uint8_t)dxl.second,
                                     pos_control->address,
                                     pos_control->data_length,
                                     &read_position,
                                     &log);
      if (result == false)
      {
  ...
  if (is_moving_ == false)
  {
    ...
    else if (dxl_wb_->getProtocolVersion() == 1.0f && true)
    {
      ...
      for (auto const &dxl : dynamixel_)
      {
        ...
        uint16_t length_of_data = pos_control->data_length +
                                  pos_velocity->data_length +
                                  pos_current->data_length;
        ...
        dynamixels_states_.dynamixel_state[(uint8_t)dxl.second].present_position = dynamixel_state[dxl_cnt].present_position;
        dynamixels_states_.dynamixel_state[(uint8_t)dxl.second].present_velocity = dynamixel_state[dxl_cnt].present_velocity;
        dynamixels_states_.dynamixel_state[(uint8_t)dxl.second].present_current = dynamixel_state[dxl_cnt].present_current;
        dynamixels_states_.dynamixel_state[(uint8_t)dxl.second].id = (uint8_t)dxl.second;
        dynamixels_states_.dynamixel_state[(uint8_t)dxl.second].name = dxl.first;
...
void DynamixelController::writeCallback(const ros::TimerEvent &)
{
  ...
  static ros::Time when_start_move = ros::Time::now();
  ...
  if (is_moving_ == true)
  {
    if (point_cnt == 0)
    {
      when_start_move = ros::Time::now();
      ROS_WARN("when_start_move seconds: %d, and nanoseconds: %d", when_start_move.sec, when_start_move.nsec);
    }
    
    // Real-Time control
    ros::Duration loop_time = ros::Time::now() - when_start_move;
    if (loop_time < jnt_tra_msg_->points[point_cnt].time_from_start)
    {
      ROS_WARN("Too fast! Wait...");
      return;
    }
    ...
    for (uint8_t index = 0; index < id_cnt; index++)
    {
      ...
      int32_t vel = dxl_wb_->convertVelocity2Value(id_array[index], abs(jnt_tra_msg_->points[point_cnt].velocities.at(index)));
      // ROS_INFO("Setting motor %d to position %d", id_array[index], dynamixel_position[index]);
      uint16_t num = dxl_wb_->getModelNumber(id_array[index], &log);
      if (num == MX_64_2)
      {
        result = dxl_wb_->itemWrite(id_array[index], "Profile_Acceleration", 10, &log);
        if (vel == 0)
          vel = 10;
        if (vel > 25)
          vel = 25;
        // vel = 25;
        result = dxl_wb_->itemWrite(id_array[index], "Profile_Velocity", vel, &log);
        result = dxl_wb_->itemWrite(id_array[index], "Goal_Position", dynamixel_position[index], &log);
        ROS_INFO("Setting dynmixel %d with pos %d vel %d", id_array[index], dynamixel_position[index] , vel);
      }
      else
      {
        // Goal_Position unit: 0.005061455 rad/un
        // ROS_INFO("Setting dynmixel %d with from %f to moving vel to %d", id_array[index],jnt_tra_msg_->points[point_cnt].velocities.at(index),vel);
        if (vel == 0)
          vel = 800;
        // if (vel > 10) vel = 10;
        result = dxl_wb_->itemWrite(id_array[index], "Moving_Speed", vel, &log);
        result = dxl_wb_->itemWrite(id_array[index], "Goal_Position", dynamixel_position[index], &log);
        ROS_INFO("Setting dynmixel %d with pos %d vel %d", id_array[index],dynamixel_position[index] ,vel);
      }
    }
    ROS_INFO("Sending command to motors...");
    // result = dxl_wb_->syncWrite(SYNC_WRITE_HANDLER_FOR_GOAL_POSITION, id_array, id_cnt, dynamixel_position, 1, &log);

        // Check if all joints reaches their goals
    // for (uint8_t index = 0; index < id_cnt; index++)
    // {
    //   int32_t present_pos = dynamixels_states_.dynamixel_state[id_array[index]].present_position;
    //   int32_t goal_pos = dxl_wb_->convertRadian2Value(id_array[index], jnt_tra_msg_->points[point_cnt].positions.at(index));
    //   int32_t error_pos = abs(present_pos - goal_pos) ;
    //   if (abs(present_pos - goal_pos) > 10)
    //   {
    //     ROS_WARN("Sorry! Motor %d did reached it goal yet! Error of %d - %d = %d. Wait...", id_array[index], present_pos, goal_pos, error_pos);
    //     return;
    //   }
    // }
    ...
    // Real-Time control
    loop_time = ros::Time::now() - when_start_move;
    ROS_INFO("loop duration seconds: %d, and nanoseconds: %d", loop_time.sec, loop_time.nsec);
    for (uint32_t index = point_cnt; index < jnt_tra_msg_->points.size(); index++)
    {
      if (loop_time > jnt_tra_msg_->points[index].time_from_start)
      {
        point_cnt = index + 1;
        break;
      }
    }
    ROS_INFO("Going to point %d/%d", point_cnt, jnt_tra_msg_->points.size());
    ...
    if (point_cnt >= jnt_tra_msg_->points.size())
    {
      ...
      ROS_INFO("\33[93mComplete Execution\33[0m");
    }
    // }
  }
    ...
    if (id_cnt != 0)
    {
      ...
      while (cnt < msg->points.size())
      {
        ...
        if (use_moveit_ == true)
        {
          ...
          jnt_tra_point_msg.time_from_start = msg->points[cnt].time_from_start;
        ...
        else
        {
          ...
          ROS_INFO("Moving timer: %f", move_time);
